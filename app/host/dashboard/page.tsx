...
export default async function HostDashboardPage() {
  console.log("[v0] Host Dashboard - starting to load")

  try {
    const user = await getSession()
    console.log("[v0] Host Dashboard - user session:", user ? "exists" : "null")
    ...
    console.log("[v0] Host Dashboard - fetching organization for user:", user.id)
    const organizations = await sql`
      SELECT * FROM public.host_organizations WHERE user_id = ${user.id}
    `
    console.log("[v0] Host Dashboard - organizations found:", organizations.length)

    const organization = organizations[0]

    // If no organization exists, redirect to create profile
    if (!organization) {
      console.log("[v0] Host Dashboard - no organization, redirecting to profile")
      redirect("/host/profile")
    }

    console.log("[v0] Host Dashboard - organization:", {
      id: organization.id,
    ...
    console.log("[v0] Host Dashboard - fetching opportunities")
    const opportunities = await sql`
      SELECT * FROM public.opportunities 
      WHERE host_organization_id = ${organization.id}
      ORDER BY created_at DESC
    `
    console.log("[v0] Host Dashboard - opportunities found:", opportunities.length)

    // Fetch applications to this organization's opportunities
    console.log("[v0] Host Dashboard - fetching applications")
    ...
    `
    console.log("[v0] Host Dashboard - applications found:", applications.length)

    const rating = organization.rating ? Number(organization.rating) : 0
    const reviewCount = organization.review_count || 0
    ...
    console.log("[v0] Host Dashboard - rendering page")
    ...
    console.error("[v0] Host Dashboard - error:", error)
    console.error("[v0] Host Dashboard - error details:", {
